...
for iEvt in range(tree.GetEntries()):
  ...
  ## Lists of offline muon indices
  iOff, iL1= [], []
  muon_indices = list(range(tree.nMuon))
  muon_indices.sort(key=lambda i: tree.Muon_pt[i], reverse=True)
  for iOff in muon_indices:
    recoEta = tree.Muon_eta[iOff]
    recoPhi = tree.Muon_phi[iOff]
    recoPt   = tree.Muon_pt[iOff]
    print("the offline muon has", recoPt)
    ...
    recoIsTight = tree.Muon_tightId[iOff]
    recoIsLoose = tree.Muon_looseId[iOff]
    ...
    recoDxy = tree.Muon_dxy[iOff]
    recoDz  = tree.Muon_dz[iOff]
    if abs(recoDxy) > 0.2 or abs(recoDz) > 0.5: continue
# Create a list of L1 muon indices and sort them based on pt
    l1muon_indices = list(range(tree.nL1Mu))
    l1muon_indices.sort(key=lambda i: tree.L1Mu_pt[i], reverse=True)

# Now, l1muon_indices contains the indices of L1 muons in pt order.
    for iL1 in l1muon_indices:
      #if tree.L1Mu_hwQual[iL1] < L1_QUAL: continue
      l1_eta = tree.L1Mu_etaAtVtx[iL1]
      l1_phi = tree.L1Mu_phiAtVtx[iL1]
      l1_pt = tree.L1Mu_pt[iL1]
      if CalcDR( l1_eta, l1_phi, recoEta, recoPhi ) > MAX_dR_L1_P or tree.L1Mu_bx[iL1] != 0 : continue
      n_matched+=1
      if n_matched > 1 : continue
      print(n_matched)
      print("the matched muon has", l1_pt)
      for tf in trig_TF:
