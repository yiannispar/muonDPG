  ...
  for TF in trig_TF.keys():
    for WP in trig_WP.keys():
      for pt in trg_pt[WP]:
        ...
        h_misid_phi[key].SetDirectory(0)
## ================================================
        ...
        for WP in trig_WP.keys():
            for pt in trg_pt[WP]:
                ...
                for iL1 in range(tree.nL1Mu):
                    ...
                    if tree.L1Mu_bx[iL1] != 0: continue           # Bunch Crossing = 0
                    if CalcDR( l1_eta, l1_phi, recoEta, recoPhi ) < 0.1 and tree.L1Mu_hwQual[iL1] >= trig_WP[WP][0] and tree.L1Mu_pt[iL1] >= pt:
                        if recoCharge == l1_charge:
                            wrong_charge = False
                        else:
                            wrong_charge = True

                        h_misid_phi[key].Fill(wrong_charge,recoPhi)
    ...
    for tf in trig_TF:
        for WP in trig_WP.keys():
            for pt in trg_pt[WP]:
                ...
                #h_misid_phi[key].Write()

                h_passed_phi = h_misid_phi[key].GetPassedHistogram()
                h_passed_phi.SetName(key + "_phi_passed")
                h_passed_phi.Write()
                h_total_phi = h_misid_phi[key].GetTotalHistogram()
