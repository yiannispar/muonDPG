...
##INFO: The way we read through the different lists of muons, there is high chance that there are multiple matches for the same offline muon (~3.7%). To prevent this, we sort the muons with the pT and when we find a L1 muon to match, we continue to the next offline muon. The keyword brute represents the brute forcing of only one match. In this script we create both cases for comparison. 
...
## Same plots forcing only one match 
h_dr_brute ={}
h_misid_pt_brute = {}
h_misid_pt_2_brute={}
h_misid_phi_eta_brute ={}
h_misid_l1_phi_eta_brute ={}
...
for TF in trig_TF.keys():
  ...
  ## Same plots forcing only one match 
  h_dr_brute[key] = ROOT.TH1F("h_dr_brute%s" % key,";#DeltaR;",100,0,1)

  h_misid_pt_brute[key] = ROOT.TEfficiency("h_misid_pt_brute%s" % key,";Reco p_{T} [GeV];Charge misidentification", len(scale_pt_temp)-1,  scale_pt)
  h_misid_pt_2_brute[key] = ROOT.TEfficiency("h_misid_pt2_brute%s" % key,";Reco p_{T} [GeV];Charge misidentification", len(scale_pt_temp_2)-1,  scale_pt_2)
  h_misid_phi_eta_brute[key] = ROOT.TEfficiency("h_misid_phi_eta_brute%s" % key,";#eta;#phi [rad]", eta_bins[0], eta_bins[1], eta_bins[2], phi_bins[0], phi_bins[1], phi_bins[2])
  h_misid_l1_phi_eta_brute[key] = ROOT.TEfficiency("h_misid_l1_phi_eta_brute%s" % key,";#eta;#phi [rad]", eta_bins[0], eta_bins[1], eta_bins[2], phi_bins[0], phi_bins[1], phi_bins[2])
      
  h_misid_pt_brute[key].SetDirectory(0)
  h_misid_pt_2_brute[key].SetDirectory(0)
  h_misid_phi_eta_brute[key].SetDirectory(0)
  h_misid_l1_phi_eta_brute[key].SetDirectory(0)
#------------------------------------------------------------------------------

## Plots for understanding better the problem
h_n_matched = ROOT.TH1I("n_matched", ":# of matched L1 muons", 10, 0, 10)
#h_multiple_pt =ROOT.TH1F("pT", "pT", 100, 0, 200)
...
for iEvt in range(tree.GetEntries()):
  ...
  ## Sort the offline muons according to the pT
  Muon_pt = list(tree.Muon_pt)
  Muon_pt.sort(reverse=True)
  ...
  for i in range(tree.nMuon):
    ...
    print("*"*10)
    print("Event " + str(iEvt)+": Offline muon " + str(i) + " has " +str(n_matched) + " L1 muons matched")
    if n_matched > 0:
      h_n_matched.Fill(n_matched)
    # if n_matched > 1:
    #   h_multiple_pt.Fill(recoPt)

  #------------------------------------------------------------------------------

  # Second loop for brute forcing the number of matched to be equal to 1
  for j in range(tree.nMuon):

    recoCharge = tree.Muon_charge[j]
    recoPt = tree.Muon_pt[j]

    ##Compute offline muon coordinates
    recoEta = tree.Muon_eta[j]
    recoPhi = tree.Muon_phi[j]
    ...
    ## Require muon to pass Muon POG tight ID
    recoIsTight = tree.Muon_tightId[j]
    recoIsLoose = tree.Muon_looseId[j]
    if not recoIsTight: continue

    ## Require prompt muons
    recoDxy = tree.Muon_dxy[j]
    recoDz  = tree.Muon_dz[j]
    if abs(recoDxy) > 0.2 or abs(recoDz) > 0.5: continue
    ...
    # find matching L1 muon second time
    for iL1 in range(tree.nL1Mu):
      if tree.L1Mu_hwQual[iL1] < L1_QUAL: continue  # Minimum quality of 12
      if tree.L1Mu_bx[iL1] != 0: continue           # Bunch Crossing = 0 
      #if tree.L1Mu_pt[iL1]   < TAG_PT - 4.01: continue
      ...
      if CalcDR( l1_eta, l1_phi, recoEta, recoPhi ) < 0.1:
        ...
        for TF in trig_TF.keys():
          ...
          h_dr_brute[key].Fill(CalcDR( l1_eta, l1_phi, recoEta, recoPhi ))

          h_misid_pt_brute[key].Fill(wrong_charge,recoPt)
          h_misid_pt_2_brute[key].Fill(wrong_charge,recoPt)
          h_misid_phi_eta_brute[key].Fill(wrong_charge,recoEta,recoPhi)
          h_misid_l1_phi_eta_brute[key].Fill(wrong_charge, l1_eta, l1_phi)
...
#Second time _ brute force matching
for tf in trig_TF:
  key = tf + '_' 

  # pt2
  h_misid_pt_2_brute[key].Draw()
  h_misid_pt_2_brute[key].Write()
      
  h_passed_pt2_brute = h_misid_pt_2_brute[key].GetPassedHistogram()
  h_passed_pt2_brute.SetName(key + "_pt2_brute_passed")
  h_passed_pt2_brute.Write()
  h_total_pt2_brute = h_misid_pt_2_brute[key].GetTotalHistogram()
  h_total_pt2_brute.SetName(key + "_pt2_brute_total")
  h_total_pt2_brute.Write()

  # pt
  h_misid_pt_brute[key].Draw()
  h_misid_pt_brute[key].Write()

  h_passed_pt_brute = h_misid_pt_brute[key].GetPassedHistogram()
  h_passed_pt_brute.SetName(key + "_pt_brute_passed")
  h_passed_pt_brute.Write()
  h_total_pt_brute = h_misid_pt_brute[key].GetTotalHistogram()
  h_total_pt_brute.SetName(key + "_pt_brute_total")
  h_total_pt_brute.Write()

  # phi_eta
  h_misid_phi_eta_brute[key].Draw()
  h_misid_phi_eta_brute[key].Write()

  h_passed_phi_eta_brute = h_misid_phi_eta_brute[key].GetPassedHistogram()
  h_passed_phi_eta_brute.SetName(key + "_phi_eta_brute_passed")
  h_passed_phi_eta_brute.Write()
  h_total_phi_eta_brute = h_misid_phi_eta_brute[key].GetTotalHistogram()
  h_total_phi_eta_brute.SetName(key + "_phi_eta_brute_total")
  h_total_phi_eta_brute.Write()

  # l1_phi_eta
  h_misid_l1_phi_eta_brute[key].Draw()
  h_misid_l1_phi_eta_brute[key].Write()

  h_passed_l1_phi_eta_brute = h_misid_l1_phi_eta_brute[key].GetPassedHistogram()
  h_passed_l1_phi_eta_brute.SetName(key + "l1_phi_eta_brute_passed")
  h_passed_l1_phi_eta_brute.Write()
  h_total_l1_phi_eta_brute = h_misid_l1_phi_eta_brute[key].GetTotalHistogram()
  h_total_l1_phi_eta_brute.SetName(key + "l1_phi_eta_brute_total")
  h_total_l1_phi_eta_brute.Write()

  h_dr_brute[key].Draw()
  h_dr_brute[key].Write()
